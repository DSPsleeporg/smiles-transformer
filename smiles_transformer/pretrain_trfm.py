...
class TrfmSeq2seq(nn.Module):
    def __init__(self, in_size, hidden_size, out_size, n_layers, dropout=0.1):
        ...
        self.trfm = nn.Transformer(d_model=hidden_size, nhead=4, num_encoder_layers=4, num_decoder_layers=4, dim_feedforward=1024)
...
def main():
    ...
    model = TrfmSeq2seq(len(vocab), args.hidden, len(vocab), 4).cuda()
    ...
    for e in range(1, args.n_epoch):
        ...
        if not best_val_loss or val_loss < best_val_loss:
            ...
            if not os.path.isdir(".save"):
                os.makedirs(".save")
            torch.save(model.state_dict(), './.save/trfm_%d.pkl' % (e))
